...
bool Adafruit_SSD1305::begin(uint8_t addr, bool reset) {
  ...
  static const uint8_t init_128x32[] = {
    ...
    0x2E,                                // ??
    SSD1305_SETCONTRAST, 0x32,           // 0x81, 0x32
    SSD1305_SETBRIGHTNESS, 0x10,         // 0x82, 0x10
    SSD1305_SEGREMAP | 0x00,
    SSD1305_NORMALDISPLAY,               // 0xA6
    SSD1305_SETMULTIPLEX, 0x3F,          // 0xA8, 0x3F (1/64)
    SSD1305_MASTERCONFIG, 0x8E,          // external vcc supply
    SSD1305_COMSCANDEC,
    SSD1305_SETDISPLAYOFFSET, 0x40,      // 0xD3, 0x40
    SSD1305_SETDISPLAYCLOCKDIV, 0xF0,    // 0xD5, 0xF0
    SSD1305_SETAREACOLOR, 0x05,
  ...
  static const uint8_t init_128x64[] = {
    ...
    0x2E,                          //??
    SSD1305_SETCONTRAST, 0x32,                 // 0x81, 0x32
    SSD1305_SETBRIGHTNESS, 0x80,                 // 0x82, 0x80
  ...
  if ((HEIGHT == 64) && (! oled_commandList(init_128x64, sizeof(init_128x64)))) {
    return false;
  } else if (HEIGHT == 32 && (! oled_commandList(init_128x32, sizeof(init_128x32)))) {
    return false;
  }
